...
```{r}
ST_soil  <- subset_samples(ST, sample_type %in% c("SoilDeep","SoilShallow","PlantRoot"))
  ST.uni <- get_unifrac(ST_soil)
  ST.ord <- ordinate(ST_soil, "NMDS", "bray")
  
  #plot
  p <- plot_ordination(ST_soil, ST.ord, type="samples", color = "shore_dist", shape = "sample_type", title= paste("Nonhost, NMDS, Bray Curtis, Soils, roots"))
  p <- p  + 
    scale_shape_discrete(solid=T)+
    scale_color_gradient2(low = "green", mid = "blue", high = "red") 
  print(p)
...
```{r}
ST_mid  <- subset_samples(ST, (transect_name %in% c("STFalls","STAboveFalls","STDrumRoadMakai","STGardens") &  sample_type %in% c("SoilDeep","SoilShallow")))
  ST.uni <- get_unifrac(ST_mid)
  ST.ord <- ordinate(ST_mid, "NMDS", ST.uni)
  
  #plot
  p <- plot_ordination(ST_mid, ST.ord, type="samples", color = "shore_dist", shape = "sample_type", title= paste("Nonhost, NMDS, Bray Curtis,"))
  p <- p  + 
    scale_shape_discrete(solid=T)+
    scale_color_gradient2(low = "green", mid = "blue", high = "red") 
  print(p)
