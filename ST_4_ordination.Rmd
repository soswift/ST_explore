...
get_unifrac <- function(physeq, unifrac_matrix = m, sample_id_column = NULL){
  # subsets unifrac matrix columns and rows by set of ids
  
  # if no sample id column specified, use sample names
  if(is.null(sample_id_column)){
  ids <- sample_names(physeq)
  }else{
  dat <- sample_data(physeq)
...
ST.uni <- get_unifrac(ST_16S_phy, sample_id_column = "sequence_id")
...
paired_ordination <- function(microbe_phy,
  ...
  p_micro <- p_micro + scale_shape_discrete(solid = T) + uniform_gradient_color()
  print(p_micro)
  ...
  p_chem <- p_chem + scale_shape_discrete(solid = T) + uniform_gradient_color()
  print(p_chem)
  
  
  # TODO calculate mantel
  ## arrange MS distance matrix to match the microbe sample order
  # chem_dist_mat <- ST_chem.dist %>% as.matrix()
  # re_order      <- match(colnames(chem_dist_mat), microbe_phy@sam_data$MS_barcode)
  # chem_dist_mat <- chem_dist_mat[re_order, re_order]
  # 
  # mantel(ST.dist, ST_chem.dist)
  # 
  # return plots as list
  
  plot_list <- list(p_micro,p_chem)
  names(plot_list) <- c( "micro", "chem" )
  return(plot_list)
...
p$marine <- paired_ordination(ST, ST_chem, description = "Marine")



# Soils and Sediments
ST  <- subset_samples(ST_16S_phy,
                      habitat != "Marine" & sample_type %in% c("SoilShallow", "SoilDeep", "Sediment"))
...
```{r}
grid.arrange(p$all$micro        , p$all$chem,
             p$marine$micro     , p$marine$chem,
             p$soil_sed$micro   , p$soil_sed$chem,
             p$plant_roots$micro, p$plant_roots$chem,
             p$water$micro      , p$water$chem,
             p$invert$micro     , p$invert$chem,
             nrow = 6, ncol = 2
             )

g <-arrangeGrob(p$all$micro        , p$all$chem,
                p$marine$micro     , p$marine$chem,
                p$soil_sed$micro   , p$soil_sed$chem,
                p$plant_roots$micro, p$plant_roots$chem,
                p$water$micro      , p$water$chem,
...
ggsave("outputs/figures/combined_microbe_metabolite_mds.png", g, width = 10, height = 20)
```
