...
```{r import_unifrac}
# Read in unifrac distances
x <- scan("data/raw/sequence/all_unifrac_100.weighted.dist")
x <- x[-1]
...
# write to pdf
pdf("outputs/figures/all_samples_habitat_nmds.pdf",width = 8, height = 6)
print(p)
dev.off()
#### by transect

p <- plot_ordination(ST_16S_phy, ST.ord, type="samples", color = "transect_name", shape = "host", title="Transect/Host NMDS, Unifrac, All Samples, All OTUs")
p <- p + scale_shape_discrete(solid = T)

print(p)

# write to pdf
pdf("outputs/figures/all_samples_nmds.pdf",width = 8, height = 6)
print(p)
dev.off()
...
for ( i in sample_types ){
 ...
 # plot MDS1 against shore_dist
  ...
  p <-ggplot(MDS_cor,  aes(shore_dist, MDS1, color = shore_dist)) +
    geom_point(size = 4) +
    scale_color_gradient2(low = "green", mid = "blue", high = "red") + 
    labs(caption = paste("R^2 = ", round(r_sq, 3), "p =", p_val), title = i)+
  ...
  if(  p_val <0.01  ){
    ...
    # if regression is positive, flip the plot and add regresssion line
    
    if(mod1$coefficients[2] > 0 ){
      
    p <- p+ geom_abline(slope = mod1$coefficients[2]*-1, intercept = mod1$coefficients[1]) + scale_x_reverse()
  ...
  png(paste("outputs/figures/mds_cor_" , i , ".png", sep = ""), width = 700, height = 600)
  print(p)
  dev.off()
...
ST_soil  <- subset_samples(ST, sample_type %in% c("SoilDeep","SoilShallow","PlantRoot","Sediment"))
  ...
  p <- plot_ordination(ST_soil, ST.ord, type="samples", color = "shore_dist", shape = "sample_type")
  p <- p  + 
    scale_shape_discrete(solid=T)+
    scale_color_gradient2(low = "green", mid = "blue", high = "red") +
    theme(title = element_text(size = 16, face = "bold"), axis.text=element_text(size=12),legend.text = element_text(size = 20))+
 ...
 # Cor plot shore_dist and NMDS1
  p <- ggplot(MDS_cor,  aes(shore_dist, NMDS1, color = shore_dist)) +
    ...
    theme(title = element_text(size = 16, face = "bold"),legend.text = element_text(size = 20), axis.text.x = element_blank() ) +
    coord_flip()
  print(p)
  
  ggsave("outputs/figures/soils_etc_cor.png", width = 10, height = 10)
...
# # check
 ...
 ST_fam_roots <- prune_taxa(names(sort(taxa_sums(ST_fam_roots),TRUE)[1:300]), ST_fam_roots)
 plot_heatmap(ST_fam_roots, sample.label="site_order")
