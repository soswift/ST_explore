...
#'               theta = thetas, phi = 45)
#' 
save_3d_gif <- function(hillshade, heightmap, file, duration = 5, sites_spheres, ...) {
  require(rayshader)
  require(magick)
  require(rgl)
  require(gifski)
  ...
  temp_dir <- tempdir()
  img_frames <- file.path(temp_dir, paste0("frame-", seq_len(n_frames), ".png")) # png
  on.exit(unlink(img_frames))
  message(paste("Generating", n_frames, "temporary .png images...")) # png
  for (i in seq_len(n_frames)) {
   ...
   #rgl::rgl.postscript(img_frames[i], fmt = "svg")
    }
